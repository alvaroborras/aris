...
Install: add to .claude/settings.json or .codex/hooks.json (matcher "Bash"); see
templates/claude-hooks/corpus_write_guard.json. Exit 2 = block + reason to the model.

This is specifically a Bash-write guard. It is not a complete sandbox and does not
gate Codex's ``apply_patch`` tool; the human landing gate and fresh review remain
authoritative for corpus changes.
"""
...
_CORPUS = r"(?:\./)?(?:(?:\.agents/)?skills|(?:skills/)?shared-references|tools|templates|plugins)/"
...
_WRITE_OPS = [
    ...
    re.compile(
        r"\b(?:cp|mv|install|rsync|ln)\b[^|;&]*\s\"?'?" + _CORPUS
    ),  # cp/mv/... corpus (as dest)
    re.compile(r"\btruncate\b[^|;&]*" + _CORPUS),
    re.compile(r"\btouch\s+[^|;&]*" + _CORPUS),  # touch corpus (create)
    re.compile(
        r"\.write_(?:text|bytes)\b[^|;&]*" + _CORPUS
    ),  # Path(...).write_text into corpus
    re.compile(
        _CORPUS + r"[^'\"]*['\"]\s*\)\s*\.write_(?:text|bytes)"
    ),  # Path('corpus..').write_text
    re.compile(
        r"\b(?:python3?|perl|ruby|node)\b[^|;&]*open\([^)]*" + _CORPUS
    ),  # open('skills/..','w')
]
...
def main(data: dict | None = None) -> int:
    if data is None:
        try:
    ...
    if hit:
        sys.stderr.write(
            ...
            "lands them with the apply_patch/Write/Edit tools after the fresh jury + human gate.\n"
        )
