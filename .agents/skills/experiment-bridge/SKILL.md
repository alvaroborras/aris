...
Implement and deploy experiments from plan: **the user's request**
...
- **BACKENDS = local | ssh | vast | modal** — Preserve the upstream workflow backend lifecycle. Vast.ai and Modal routes are first-class when configured; do not silently fall back to local execution if the user requested either backend.
